...
const { getRole, createToken, getUserFullName } = require("../../middleware/auth");
...
router.get("/", async (req, res) => {
  try {
    ...
    const spectators = await knex("users")
      ...
      .select("users.id", "users.email", "users.created_at", "users.email_verified", "users.last_login_at", "spectators.first_name", "spectators.last_name")
...
router.get("/invited", async (req, res) => {
  try {
    ...
    const invitedSpectators = await knex("invited_spectators").select("email", "created_at", "expires_at");
...
router.post("/register", async (req, res) => {
  ...
  // Input validation
  if (!spectator.email || !spectator.password || !spectator.first_name || !spectator.last_name || !spectator.token) {
  ...
  try {
    await knex.transaction(async (trx) => {
      ...
      const existingUser = await trx('users').where({ email: spectator.email }).first();
      if (existingUser) {
      ...
      if (!tokenRecord) {
        return res.status(400).json({ error: "No invitation token found for this email" });
      }
      ...
      const tokenIsValid = await bcrypt.compare(spectator.token, tokenRecord.token_hash);
      if (!tokenIsValid) {
        return res.status(400).json({ error: "Invalid invitation token" });
      }

      // Hash the password
      const passwordHash = await bcrypt.hash(spectator.password, Number(saltRounds));
...
router.post("/invite", async (req, res) => {
  ...
  try {
    ...
    const existingInvitation = await knex("invited_spectators").where({ email }).first();
    ...
    const invitationTokenHash = await bcrypt.hash(invitationToken, Number(saltRounds));
    const newSpectator = {
      email,
