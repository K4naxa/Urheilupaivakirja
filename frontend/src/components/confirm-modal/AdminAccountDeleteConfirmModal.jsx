...
const AdminAccountDeleteConfirmModal = ({
  onDecline,
  handleLogout,
  text,
  ...
  const deleteAdmin = useMutation({
    mutationFn: (password) => teacherService.deleteTeacherUserSelf(password),
    onError: (error) => {
      ...
      if (error.response) {
        ...
        switch (error.response.status) {
          ...
          case 403:
            errorMessage =
              "Käyttäjäsi on ainut opettaja eikä sitä voi siksi poistaa. Luo uusi opettajatunnus ennen poistoa.";
            break;
          case 500:
            errorMessage =
...
export default AdminAccountDeleteConfirmModal;
